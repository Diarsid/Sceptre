...
import static diarsid.sceptre.impl.AnalyzeUtil.processCluster;
...
import static diarsid.sceptre.impl.MatchType.MATCH_TYPO_1;
import static diarsid.sceptre.impl.MatchType.MATCH_TYPO_2;
import static diarsid.sceptre.impl.MatchType.MATCH_TYPO_3_1;
import static diarsid.sceptre.impl.MatchType.MATCH_TYPO_3_2;
import static diarsid.sceptre.impl.MatchType.MATCH_TYPO_3_3;
...
import static diarsid.sceptre.impl.weight.WeightElement.CHAR_AFTER_PREVIOUS_SEPARATOR_AND_CLUSTER_LOOKS_LIKE_WORD;
import static diarsid.sceptre.impl.weight.WeightElement.CHAR_AFTER_PREVIOUS_SEPARATOR_AND_CLUSTER_LOOKS_LIKE_WORD_AT_PATTERN_START;
...
class PositionsAnalyze {
    ...
    ClusterStepTwo currStepTwoCluster = new ClusterStepTwo();
    ClusterStepTwo prevStepTwoCluster = new ClusterStepTwo();
//    ClusterStepTwo lastSavedStepTwoCluster = new ClusterStepTwo();
    ...
    void analyzePositionsClusters() {
        ...
        if ( this.clusters.nonEmpty() ) {
            ...
            if ( totalTearDown > 0 ) {
                ...
                logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "               [TEARDOWN] total : %s", totalTearDown);
            }            
        }
        ...
        if ( this.clustersQty > 1 && this.allClustersInconsistency == 0 ) {
            if ( this.separatorsBetweenClusters > 0 ) {
                if ( this.clusters.distanceBetweenClusters() == this.separatorsBetweenClusters ) {
                    logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "               [weight] all clusters are one pattern, can be regarded as one cluster!");
                    this.clustersQty = 1;
                }
            } else {
                ...
                if ( firstSeparatorAfterFirstPosition == lastPosition + 1 ) {
                    if ( nonNull(lastSeparatorBeforeFirstPosition) ) {
                        if ( lastSeparatorBeforeFirstPosition == firstPosition - 1 ) {
                            logAnalyze(
                                    AnalyzeLogType.POSITIONS_CLUSTERS,
...
//                this.positionsWeight = this.positionsWeight - 15.5;
    ...
    private void doWhenOnlyNextCharacterIsSeparator() {
        ...
        if ( this.previousClusterLastPosition != POS_UNINITIALIZED && ! this.previousClusterEndsWithSeparator && this.previousClusterOrdersIsConsistent ) {
            if ( ! this.areSeparatorsPresentBetween(this.previousClusterLastPosition, this.currentPosition) ) {
                this.weight.add(-6, PREVIOUS_CLUSTER_AND_CURRENT_CHAR_BELONG_TO_ONE_WORD);
            } 
        }
    }
    
    private boolean areSeparatorsPresentBetween(final int fromExcl, final int toExcl) {
        logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "               [weight] ...searching for separators between %s and %s", fromExcl, toExcl);
        ...
        for (int pointer = fromExcl + 1; pointer < toExcl; pointer++) {
            if ( isWordsSeparator(variantText.charAt(pointer)) ) {
                logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "               [weight] separator found - %s", pointer);
                return true;
            }
        }
        return false;
    ...
    private void analyzeAllClustersOrderDiffs() {
        ...
        if ( orderMeansDifferentFromZero > 0 ) {
            if ( allClustersHaveLength2 ) {
                if ( this.clusters.quantity() == 1 ) {
                    this.allClustersInconsistency = orderMeansDifferentFromZero * 10;
                    this.weight.add(allClustersInconsistency, CLUSTERS_ORDER_INCOSISTENT);
                }
                else {
                    this.allClustersInconsistency = orderMeansDifferentFromZero * 4;
                    this.weight.add(allClustersInconsistency, CLUSTERS_ORDER_INCOSISTENT);
                }
            }
            else {
    ...
    private void analyzeAllClustersPlacing() {
        ...
        if ( estimate(this.weight.sum() + this.data.weight.sum()).equals(BAD) ) {
            ...
            logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "    [cluster placing] positions weight is too bad for placing assessment");
    ...
    private void processCurrentPatternCharOf(int currentPatternCharIndex, int charsRemained) {
        ...
        if ( positions[currentPatternCharIndex] != POS_UNINITIALIZED ) {
            logAnalyze(AnalyzeLogType.POSITIONS_SEARCH, "          [info] '%s' in pattern is already found - %s", this.currentChar, positions[currentPatternCharIndex]);
            return;
        }
        ...
        characterSearching : while ( currentPatternCharPositionInVariant >= 0 && continueSearching ) {
            ...
            if ( findPositionsStep.equalTo(STEP_2) ) {
            ...
            if ( ! positionAlreadyFilled ) {
                ...
                if ( findPositionsStep.typoSearchingAllowed() ) {
                            ...
                            if ( ! data.variantSeparators.contains(nextNextPosition - 1) ) {
                                ...
                                if ( nextCharInPattern == nextNextCharInVariant ) { // TODO here something about _x..ab.. word-cluster logic
                                    ...
                                    if ( currentCharIsVariantWordStart ) {
                                        ...
                                        int fiPattern = -1;
                                        int fjVariant = -1;
                                        boolean isFilledfjVariant = false;
                                        boolean isPositionSetfiPattern = false;
                                        ...
                                        patternLookup: for (; iPattern <= limitPattern; iPattern++) {
                                            ...
                                            variantLookup: for (int jVariant = iVariant; jVariant <= limitVariant; jVariant++) {
                                                ...
                                                if ( patternCh == variantCh ) {
                                                    ...
                                                    matches++;
                                                    if ( matches == 1 ) {
                                                        fpatternCh = patternCh;
                                                        fiPattern = iPattern;
                                                        fjVariant = jVariant;
                                                        isFilledfjVariant = filledPositions.contains(jVariant);
                                                        isPositionSetfiPattern = isPositionSetAt(iPattern);
                                                    }
                                                    else if ( matches > 1 ) {
                                                        if ( matches == 2 ) {
                                                            currStepTwoCluster.add(
                    ...
                    Integer separatorIndexBeforeWord = this.data.variantSeparators.lower(currentPatternCharPositionInVariant);
                    Integer separatorIndexAfterWord = this.data.variantSeparators.higher(currentPatternCharPositionInVariant);
                    ...
                    int foundInWord = calculateFoundBetween(wordStartIndex, wordEndIndex);

                    currStepTwoCluster.applyCurrentWordBounds(wordStartIndex, wordEndIndex, foundInWord);
                ...
                if ( findPositionsStep.equals(STEP_1) ) {
                    ...
                    if ( maySearchAsCharInWord && currStepOneCluster.isNotSet() ) {
                        ...
                        if ( wordStartIndex <= currentPatternCharPositionInVariant && currentPatternCharPositionInVariant <= wordEndIndex  ) {
                            logAnalyze(AnalyzeLogType.POSITIONS_SEARCH, "          [candidate] '%s'(%s in variant) is single char in same word as prev cluster", currentChar, currentPatternCharPositionInVariant);
                            currStepOneCluster.setMain(currentPatternCharIndex, currentPatternCharPositionInVariant);
                            currStepOneCluster.markCharAsWordEnd();
                        }
...
//                        }  
            ...
            if ( findPositionsStep.typoSearchingAllowed() ) {
                if ( currStepTwoCluster.isSet() ) {
                    if ( prevStepTwoCluster.isSet() ) {
                        ...
                        logAnalyze(AnalyzeLogType.POSITIONS_SEARCH, "        [ACCEPT POSITIONS - NOTHING TO COMPARE]");
    ...
    private void fillPositionsFrom(ClusterStepOne subcluster) {
        ...
        if ( subcluster.isMainCharWordEnd() && subcluster.size() == 1 ) {
            this.meaningful++;
        }
        logAnalyze(AnalyzeLogType.POSITIONS_SEARCH, "               %s : %s", data.pattern, data.variant);
...
//        this.meaningful = this.meaningful + subcluster.meaningfulChars();
        logAnalyze(AnalyzeLogType.POSITIONS_SEARCH, "               %s : %s", data.pattern, data.variant);
    }
...
//        }
        ...
        if ( this.clusterEndsWithSeparator ) {
                    ...
                    if ( ! containsSeparatorsInVariantInSpan(this.alonePositionAfterPreviousSeparator, this.currentPosition - this.currentClusterLength + 1) ) {
                        ...
                        if ( absDiff(this.alonePositionAfterPreviousSeparator, this.clusters.lastAddedCluster().firstPosition()) == 2 ) {
                            ...
                            if ( missedPositionChar == alonePositionBeforePreviousSeparatorChar || 
                                ...
                                logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS,
                                        "         %s", log);
                            }
                        }
                    }                   
            ...
            if ( this.alonePositionAfterPreviousSeparator != POS_UNINITIALIZED &&
                 this.alonePositionAfterPreviousSeparator != POS_ERASED ) {
                Integer separator = this.data.variantSeparators.higher(this.alonePositionAfterPreviousSeparator);
                boolean isOneWord = isNull(separator) || separator > this.currentPosition;
                if ( isOneWord ) {
                    if ( this.weight.contains(PREVIOUS_CHAR_IS_SEPARATOR_CURRENT_CHAR_AT_PATTERN_START) ) {
                        this.weight.add(-9, CHAR_AFTER_PREVIOUS_SEPARATOR_AND_CLUSTER_LOOKS_LIKE_WORD_AT_PATTERN_START);
                    }
                    else {
                        this.weight.add(-6, CHAR_AFTER_PREVIOUS_SEPARATOR_AND_CLUSTER_LOOKS_LIKE_WORD);
                    }
                }
            }
        }
...
//                }
                ...
                if ( this.patternContainsClusterFoundInVariant() ) {
                    ...
                    logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "            [cluster stats] pattern DOES NOT contain cluster!");
                    this.weight.add(this.currentClusterLength, PATTERN_DOES_NOT_CONTAIN_CLUSTER);
                }
            }            
        }
    ...
    private int calculateFoundBetween(int fromIncl, int toIncl) {
        int count = 0;

        for ( Integer position : this.filledPositions ) {
            if ( fromIncl <= position && position <= toIncl ) {
                count++;
            }
        }
    ...
    void accumulateClusterPositionOrdersStats() {
        if ( this.currentClusterOrderDiffs.isEmpty() ) {
            ...
            logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "            [cluster stats] cluster is consistent");
        ...
        if ( cluster.hasOrdersDiff() ) {
            ...
            if ( cluster.hasOrdersDiffShifts() ) {
                ...
                logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "            [cluster stats] cluster has %s shifts", cluster.ordersDiffShifts());
        ...
        for (int variantPosition = 0; variantPosition < this.data.variant.length(); variantPosition++) {
            ...
            for (int patternPosition = 0; patternPosition < this.positions.length; patternPosition++) {
                ...
                if ( patternChar == variantChar ) {
                    logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "      [?] duplicate char found '%s' : %s for clustered char '%s' : %s",
    ...
    void calculateImportance() {
        ...
        logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "    [importance] clusters: %s", this.clustersImportance);
        logAnalyze(AnalyzeLogType.POSITIONS_CLUSTERS, "    [importance] non-clustered: %s", this.nonClusteredImportance);
    }
    ...
    final void clearPositionsAnalyze() {
        ...
        this.currStepOneCluster.clear();
        this.prevStepOneCluster.clear();
        this.lastSavedStepOneCluster.clear();

        this.currStepTwoCluster.clear();
        this.prevStepTwoCluster.clear();
    }
